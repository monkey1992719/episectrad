...
class ChartPlotSettingManager(models.Manager):
    def create_setting(self, user, plot, color, width, plottype):
        setting = self.create(user=user, plot=plot,
                              color=color, width=width, plottype=plottype)
    ...
    def update_setting(self, plot, color, width=1, plottype='Line'):
        ...
        if setting is None:
            setting = self.create(plot=plot, color=color,
                                  width=width, plottype=plottype)
...
class TradeIndicatorIndicator(models.Model):
    trade_indicator = models.ForeignKey(
        TradeIndicator, on_delete=models.CASCADE)
...
class TradeIndicatorPlotThreshold(models.Model):
    trade_indicator_indicator = models.ForeignKey(
        TradeIndicatorIndicator, on_delete=models.CASCADE)
    plot = models.OneToOneField(ChartPlot, on_delete=models.CASCADE)
...
class TradeIndicatorCross2(models.Model):
    trade_indicator_indicator1 = models.OneToOneField(
        TradeIndicatorIndicator, on_delete=models.CASCADE, related_name='tii2tii1')
    chart_plot1 = models.OneToOneField(
        ChartPlot, on_delete=models.CASCADE, related_name='cp2cp1')
    trade_indicator_indicator2 = models.OneToOneField(
        TradeIndicatorIndicator, on_delete=models.CASCADE, related_name='tii2tii2')
    chart_plot2 = models.OneToOneField(
        ChartPlot, on_delete=models.CASCADE, related_name='cp2cp2')
...
class TradeIndicatorCrossv(models.Model):
    trade_indicator_indicator = models.OneToOneField(
        TradeIndicatorIndicator, on_delete=models.CASCADE)
...
class IndicatorInputValue(models.Model):
    trade_indicator_indicator = models.ForeignKey(
        TradeIndicatorIndicator, on_delete=models.CASCADE)
    indicator_input = models.OneToOneField(
        IndicatorInput, on_delete=models.CASCADE)
...
class Backtest(models.Model):
    ...
    tradeindicator = models.OneToOneField(
        TradeIndicator, on_delete=models.CASCADE)
